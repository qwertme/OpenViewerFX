 ...
 *<br>precedes a sequence of characters that parses as
...
//        n2In = 0;
      ...
      if (Double.isInfinite(x)) {
       ...
       // noDigits = true;
        ...
        if (!alternateForm) {
          ...
          if (expon>=-4 && expon<precision) {
              ret = new StringBuffer(fFormatString(x).trim());
          } else {
              ret = new StringBuffer(sx);
          }
        }
    ...
    private String printXFormat(final int x) {
      ...
      if (x == Integer.MIN_VALUE) {
        ...
        final String t = Integer.toString(
